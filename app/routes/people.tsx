...
export const meta: MetaFunction<typeof loader> = ({ location, data }) => {
  ...
  const title = `People | ${siteName}`;
  const description = 'Capturing chefs and bartenders at work.';
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const query = DB.prepare(`SELECT * FROM images WHERE category = ?`).bind('people');
...
export default function PeopleRoute() {
  ...
  return (
    <div>
      <section>
        <PageHeader title="People" description="Capturing chefs and bartenders at work" />
      </section>
